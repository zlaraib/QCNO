...
def meas_counts(
    ...
    """
    Add measurement to a circuit, transpile/decompose it as needed for the target
    backend, and return shot counts.

    Inputs:
    - qc_base: Base QuantumCircuit before measurement.
    - measure: Measurement basis / label passed to add_measurement_to_circuit.
    - N_sites: Number of qubits/sites to measure.
    - backend: Execution backend.
    - backend_name: String label identifying the backend family.
    - optimization_level: Qiskit transpiler optimization level.
    - shots: Number of measurement shots.
    - euler_basis: Single-qubit Euler basis used by TwoQubitBasisDecomposer.
      Default is 'U3'.
    - two_qubit_gate: Two-qubit basis gate used by TwoQubitBasisDecomposer.
      Default is 'ECR'.

    Output:
    - counts: Measured bitstring counts.
    - isa_circuit: Backend-adapted circuit that was executed.
    """
    qc = add_measurement_to_circuit(qc_base, N_sites, measure=measure)
    ...
    if backend_name in {"manila", "ibm", "guadalupe"}:
        # Decompose each two-qubit gate into the requested two-qubit basis gate
        # plus the requested single-qubit Euler basis, then transpile to the
        # backend gate set used in this workflow.
    ...
    if backend_name in {'ionq', 'ionq_noisy_sim', 'ionq_native', 'ionq_qpu'}:
        # Decompose two-qubit gates before transpiling to the IonQ-oriented basis.
        decomposer = TwoQubitBasisDecomposer(
            basis_gate_obj,
            euler_basis=euler_basis,
        )
        ...
        isa_circuit = qc  # retained for return-value consistency
        return counts, isa_circuit
    ...
    # Non-Aer hardware / generic backend path:
